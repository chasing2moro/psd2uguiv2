    ...
    public class ButtonLayerImport : ILayerImport
    {
        ...
        public void DrawLayer(Layer layer, GameObject parent)
        {
            ...
            if (layer.layers != null)
            {
                ...
                {
                    Layer subLayer = layer.layers[imageIndex];
                    PSImage image = subLayer.image;
                    //string lowerName = image.name.ToLower();
                    if(subLayer.layers != null && subLayer.layers.Length > 0)
                    {   
                        ctrl.DrawLayer(subLayer, button.gameObject);//广州寰宇添加
                    }
                    else if (image.imageType != ImageType.Label && image.imageType != ImageType.Texture)
                    {
                        if (image.imageSource == ImageSource.Custom || image.imageSource == ImageSource.Common)
                        {
                            ...
                            var layerName = subLayer.name.ToLower();
                            if (layerName.Contains("normal"))
                            {
                                setButtonTarget(image, button, sprite);
                            }
                            else if (layerName.Contains("pressed"))
                            {
                            ...
                            else if (layerName.Contains("disabled"))
                            {
                                button.transition = UnityEngine.UI.Selectable.Transition.SpriteSwap;
                                UnityEngine.UI.SpriteState state = button.spriteState;
                            ...
                            else if (layerName.Contains("highlighted"))
                            {
                                button.transition = UnityEngine.UI.Selectable.Transition.SpriteSwap;
                                UnityEngine.UI.SpriteState state = button.spriteState;
                    ...
                    else
                    {
                        ...
                        ctrl.DrawLayer(subLayer, button.gameObject);
                    }
                }
            }
