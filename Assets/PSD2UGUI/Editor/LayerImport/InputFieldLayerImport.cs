        ...
        public void DrawLayer(Layer layer, GameObject parent)
        {
...
#else
            inputfield.caretWidth = 2;//原来是1，太窄了，看不到
            PSDImportUtility.CacheChild(inputfield.gameObject, parent); //广州寰宇添加
#endif
            ...
            for (int i = 0; i < layer.layers.Length; ++i)
            {
                ...
                if (subLayer.image != null)
                {
                    ...
                    string layerName = subLayer.name.ToLower();
                    if (image.imageType == ImageType.Label)
                    {
                        if (layerName.Contains("text"))
                        {
                            this.pSDImportCtrl.DrawLayer(subLayer, inputfield.gameObject);

                            var _text = PSDImportUtility.canvas.transform.Find(subLayer.name).GetComponent<Text>();
                        ...
                        else if (layerName.Contains("holder"))
                        {
                            this.pSDImportCtrl.DrawLayer(subLayer, inputfield.gameObject);
                            ...
                            inputfield.placeholder = PSDImportUtility.canvas.transform.Find(subLayer.name).GetComponent<Text>();
