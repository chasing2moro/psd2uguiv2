...
namespace PSDUIImporter
{
    public class ScrollViewLayerImport : ILayerImport
    {
            ...
            if (layer.layers != null)
            {
                ...
                switch (type)
                {
                    ...
                    case "GV":
                        //scrollRect.vertical = true;
                        //scrollRect.horizontal = false;
                        BuildGridVerticalScrollView(scrollRect, layer);
                        break;
                    case "GH":
                        //scrollRect.vertical = false;
                        //scrollRect.horizontal = true;
                        BuildGridHorizonScrollView(scrollRect, layer);
                        break;
                    default:
                        break;
                }
        ...
        public void BuildGridHorizonScrollView(UnityEngine.UI.ScrollRect scrollRect, Layer layer)
        {
            scrollRect.vertical = false;
            scrollRect.horizontal = true;
            ...
            var hLayout = scrollRect.content.gameObject.AddComponent<GridLayoutGroup>();  //添加水平布局组件
            if (layer.arguments.Length < 4)
            {
                Debug.LogWarning("ScrollView arguments error !");
            }
            else
            {
                ...
                if (float.TryParse(layer.arguments[1], out spacing))
                {
                    hLayout.spacing = new Vector2(spacing, spacing);
                }

                hLayout.cellSize = GetCellSize(layer);
            }
        }
        ...
        /// <param name="scrollRect"></param>
        /// <param name="layer"></param>
        public void BuildGridVerticalScrollView(UnityEngine.UI.ScrollRect scrollRect, Layer layer)
        {
            scrollRect.vertical = true;
            scrollRect.horizontal = false;
            ...
            var vLayout = scrollRect.content.gameObject.AddComponent<GridLayoutGroup>();  //添加水平布局组件
            if (layer.arguments.Length < 4)
            {
                Debug.LogWarning("ScrollView arguments error !");
            }
            else
            {
                ...
                if (float.TryParse(layer.arguments[1], out spacing))
                {
                    vLayout.spacing = new Vector2(spacing, spacing);
                }
                vLayout.cellSize = GetCellSize(layer);
            }
        }

        public static Vector2 GetCellSize(Layer ScrollViewLayer)
        {
            Layer itemLayer = null;
            if (ScrollViewLayer.layers.Length > 0)
            {
                itemLayer = ScrollViewLayer.layers[0];
                return new Vector2(itemLayer.size.width, itemLayer.size.height);
            }
            else
            {
                Debug.LogError("Can not get cell ScrollViewLayer.layers.Length" + ScrollViewLayer.layers.Length);
            }
            return Vector2.zero;
        }
    }
